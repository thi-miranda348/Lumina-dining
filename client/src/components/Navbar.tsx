            ...
            <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 md:gap-4">
                    <div className="relative flex items-center">
                        ...
                        <input
                            type="text"
                            className={`bg-surface border border-surface text-text-main text-sm rounded-sm focus:outline-none focus:border-primary transition-all duration-300 ease-in-out ${isSearchOpen ? 'w-36 md:w-48 pl-4 py-1.5 opacity-100' : 'w-0 pl-0 pr-0 py-1.5 opacity-0 pointer-events-none'
                                }`}
                            placeholder="Search..."
                        />
                    </div>
                    <Link to="/account" className="text-text-main hover:text-primary transition-colors p-2">
                        <MdPersonOutline className="text-2xl" />
                    </Link>
                    <button
                        onClick={onOpenCart}
                        className="text-text-main hover:text-primary transition-colors p-2 relative"
                    >
